...
export class ProjectEffects {
  ...
  loadProjects$: Observable<Action> = this.actions$
    ...
    // 得到两个数据，一个是上面 .map(toPayload) 的 payload，但是并不关心是什么
    // 另一个是从 withLatestFrom() 当中来的 auth
  ...
  addProject$: Observable<Action> = this.actions$
    ...
    .map(toPayload)
    .withLatestFrom(this.store$.select(fromRoot.getAuth))
    // 这次需要关心 payload，因为传递上来的是一个 project
    .switchMap(([project, auth]) => {
      // 将 project 展开，添加一个 members
      // 因为添加上一个 project 以后，就成为了该 project 的成员之一
  ...
  updateProject$: Observable<Action> = this.actions$
    ...
    .switchMap(project => this.service
      .update(project)
      .map(projects => new actions.UpdateProjectSuccessAction(projects))
  ...
  // projects 在加载的时候需要去得到当前用户的 id
  // 因为 project 是根据当前参与的用户得来的，所以需要载入 store$
  constructor(
